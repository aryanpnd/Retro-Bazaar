...
export default function Wishlist() {
  ...
  const handleDeleteWishlist = async () => {
    setDelLoading(true)
    await axios.delete(`${apiURL}/api/deleteAllFromWishlist`, {
      withCredentials: true,
    }).then((res)=>{
      setloading(false);
      setWishlistArray([])
  ...
  return (
    <>
      <Modal setModal={setModal} modal={modal} noCloseBtn={true} height={'20%'} width={'50%'} borderR={'8px'} noHead={true}>
        <div>Are you sure ?</div>
        <div className="modal-delete-btn-box">
          <button onClick={() => setModal(false)}>cancel</button>
          <button style={{background:'#c90000',color:'white'}} onClick={handleDeleteWishlist}><DeleteOutlined/>
      ...
      <div className="wishlist-header-container">
        ...
        <div className="wishlist-header-side-bar">
          <span className="wishlist-move-all-to-cart-span wishlist-heading-span-btn" onClick={() => setModal(true)}>
