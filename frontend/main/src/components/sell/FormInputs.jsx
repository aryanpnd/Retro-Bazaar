    ...
    return (
        <>
            <form className="sell-inputs-container" onSubmit={handleSubmit}>
                ...
                <input name='title' type="text" placeholder="Product title (required)" id="p-title"
                    value={titleInput.val}
                    onChange={(e) => handleInput(e, setTitleInput, 5)}
                    required />
                <label style={{ display: titleInput.error.status ? '' : 'none' }} htmlFor="p-title">{titleInput.error.message}</label>
                <br />

                <input name='description' type="text" placeholder="Product description (required)" id="p-description"
                    value={descriptionInput.val}
                    onChange={(e) => handleInput(e, setDescriptionInput, 20)}
                    required />
                <label style={{ display: descriptionInput.error.status ? '' : 'none' }} htmlFor="p-description">{descriptionInput.error.message}</label>
                <br />

                <input name='brand' type="text" placeholder="Product brand if any (leave empty if not)"
                    onChange={handleBrandChange} />
                <br />


                <select name='category' style={{ padding: "14px" }} className='options' id='categories-select' required onChange={handleCategoryChange}>
                    <option value="" disabled selected>Categories (required)</option>
                ...
                <div style={{ textAlign: 'center', marginBottom: '10px' }}>Select Location (required)</div>
